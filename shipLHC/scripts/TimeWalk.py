...
class TimeWalk(ROOT.FairTask):
    ...
    def ExecuteEvent(self, event):
        ...
        if self.referencesystem==3 and self.hasTrack and not self.simulation:
            ...
            self.td = self.muAna.GetTimingDiscriminant(hits, self.MuFilter) # Require that US1 TDC average is less than the DSH TDC average to ensure forward travelling track
    ...
    def WriteOutHistograms(self):
        ...
        if self.mode in ('zeroth', 'tof', 'tw', 'res'):
            for h in self.hists:
                ...
                # ref-t histogram only written out for mode==res
                if self.mode=='res':
