...
class reverseChannelMapping:
 " detector ID and SiPM channel nr to online board/tofpet_id/tofpet_channel "
 def Init(self,path):
    mappingPath= '/eos/experiment/sndlhc/raw_data/commissioning/US_tests_LabLausanne_2023/configuration/'
    with open(path+"/board_mapping.json") as f:
        # status, jsonStr = f.read()
        # self.boardMaps = json.loads(f.read())
    ...
    #   sndswPath = os.environ['SNDSW_ROOT']
      infile = f"{mappingPath}{system}_SiPM_mapping.csv"
      self.SiPMmap[system] = {}
      self.TofpetMap[self.names[system]] = {}
    #   if os.path.isdir(sndswPath+"/geometry"):
      ...
      for channel in self.SiPMmap[system]:
        row=self.SiPMmap[system][channel]
        self.TofpetMap[self.names[system]][int(row[2])*1000+int(row[3])] = channel
      self.offMap={}
      for s in range(1,3):
        for o in ['Left','Right']: 
...
#
 def daqChannel(self,aHit,channel=0):
    ...
    if not fDetectorID<1000000:    #scifi hit
      ...
      if not type(aHit).__name__ == 'int':
        nSiPMs = aHit.GetnSiPMs()
        nSides = aHit.GetnSides()
      else: 
        if subsystem==3: 
            nSiPMs = 1 
            if bar_number<120:    nSides = 2
            else:    nSides = 1
        else:
            nSiPMs = 8
            nSides = 2
